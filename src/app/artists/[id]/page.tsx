  ...
  return (
    <div className="bg-brand-khaki">
        <div className="bg-dot-pattern">
            <div className="container mx-auto px-4 py-12 md:py-20">
            <div className="flex flex-col md:flex-row items-center md:items-start gap-8 md:gap-12 mb-12">
                <div className="relative h-40 w-40 flex-shrink-0 overflow-hidden rounded-full shadow-lg border-4 border-white">
                {artist.avatarUrl ? (
                    <Image
                ...
                <h1 className="text-4xl md:text-5xl font-headline font-bold text-white drop-shadow-md">{artist.name}</h1>
                <GenerateBioForm artist={artist} products={products || []} />
                </div>
            </div>
            
            <h2 className="text-3xl font-headline font-bold text-center mb-10 text-white drop-shadow">Creations by {artist.name}</h2>
            {areProductsLoading ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                    {Array.from({ length: 4 }).map((_, i) => (
                        <div key={i} className="space-y-2">
                ...
                <p className="text-center text-white/80">This artist has not listed any products yet.</p>
            )}
            </div>
        </div>
    </div>
